...
def morse_text(text):
    """Encode letters and digits as International Morse; words separated by ' / '."""
    ...
    def encode_line(line):
        ...
        for ch in line.upper():
            if ch.isspace():
                if bucket:
                    words.append(" ".join(bucket))
                    bucket = []
            elif ch in MORSE:
                bucket.append(MORSE[ch])
        if bucket:
            words.append(" ".join(bucket))
        return " / ".join(words)

    return "\n".join(encode_line(ln) for ln in text.split("\n"))
...
def render_badge(text):
    """Display the greeting like a conference badge."""
    lines = text.split("\n")
...
def render_scroll(text):
    ...
    top = "  __" + "_" * (width + 2) + "__"
    edge_top = " /  " + " " * (width + 2) + "  \\"
    edge_bottom = " \\__" + "_" * (width + 2) + "__/"
    body = [f"|   {line.center(width)}   |" for line in lines]
    return "\n".join([top, edge_top, *body, edge_bottom])
...
def main():
    ...
    parser.add_argument(
        "--morse",
        action="store_true",
        help="Encode letters and digits as International Morse (words separated by ' / ').",
    )
    parser.add_argument(
        "--owl",
        action="store_true",
